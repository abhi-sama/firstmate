...
FM_PANE_FOOTER_LINES=6             # trailing non-blank capture lines treated as the harness footer: scanned for the busy signature, excluded from the stale body hash; empty, 0, or non-numeric falls back to 6
...
FM_BUSY_TAIL_LINES=8    # trailing non-blank lines scanned for that signature; wider than FM_PANE_FOOTER_LINES so a spinner displaced by claude's notice banners is still seen; zero or non-numeric falls back to 8
