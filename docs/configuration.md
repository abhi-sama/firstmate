...
## Validation monitor (config/pipeline-monitor)

`bin/fm-pipeline-monitor.sh <task-id>` is a standalone, read-only command that opens a tmux window live-tailing a worker's no-mistakes validation run; see [`pipeline-monitor.md`](pipeline-monitor.md).
It is opt-in: set the local, gitignored `config/pipeline-monitor` file to `on` and firstmate also opens it right after triggering validation (AGENTS.md section 7, "Validate"); absent or any other value keeps it manual-only.
It reads `axi status` and `axi logs` only, never `respond` or `abort`, so it cannot alter a run.
It is tmux-only for now and no-ops with a pointer on any other runtime backend.
...
FM_CLAUDE_AUTOARM_WEDGE_AFTER=900   # seconds a live Stop auto-arm owner may hold the single-flight lock with no healthy watcher behind it before a later firing takes the claim over; forced above FM_GUARD_GRACE
