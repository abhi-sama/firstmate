...
FM_HEARTBEAT_MAX=7200   # heartbeat backoff cap for a genuinely idle fleet
FM_HEARTBEAT_MAX_INFLIGHT=900   # tighter backoff cap while any task is recorded in flight; persistent secondmates do not count
...
FM_PANE_FOOTER_LINES=6             # trailing non-blank capture lines treated as the harness footer and excluded from the stale body hash; governs only that split, never the busy scan (see FM_BUSY_TAIL_LINES); empty, 0, or non-numeric falls back to 6
FM_BUSY_CLAIM_MAX=1800             # seconds a rendered busy signature may stand over a pane body producing no output before the stale wake surfaces once; empty, 0, or non-numeric falls back to 1800
...
FM_BUSY_REGEX=          # overrides the busy-pane signature set shared by the watcher, fm-crew-state's pane fallback, and the tmux helper; default is FM_TMUX_BUSY_REGEX_DEFAULT in bin/fm-tmux-lib.sh (see docs/harness-busy-signatures.md before changing it)
FM_BUSY_TAIL_LINES=8    # trailing non-blank lines scanned for that signature; wider than FM_PANE_FOOTER_LINES so a spinner displaced by claude's notice banners is still seen, which also means it reads pane body, so the signature must stay structurally anchored; zero or non-numeric falls back to 8
