...
That takeover, and any owner-claim refusal the hook could not resolve, are recorded in `state/.claude-autoarm-contended`, so a claim that blocks supervision is never again a silent exit.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight including wedged-owner takeover and the fresh-owner and healthy-long-cycle refusals, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, and exit-2 translation.
