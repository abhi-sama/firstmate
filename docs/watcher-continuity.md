...
The record is cleared only once a healthy watcher is confirmed, never by the next ordinary claim, and the session-start digest is its one reader: `bin/fm-session-start.sh` prints one bounded line beside the away-mode flag when a record exists and stays silent otherwise.
...
The same suite covers the contention record surviving a later ordinary claim until a healthy watcher clears it, a break refused by a claim that already changed hands reporting an ordinary race, and a wedged break freeing the lock name rather than stranding a lock no later acquire could reclaim.
