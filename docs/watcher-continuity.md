...
The record is cleared only once the watcher beacon has been beaten inside `FM_GUARD_GRACE`, never by the next ordinary claim, and the session-start digest is its one strictly read-only reader: `bin/fm-session-start.sh` prints one bounded line beside the away-mode flag when a record exists and stays silent otherwise.
