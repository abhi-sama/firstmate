...
For that entire span the model is not generating - it already finished the turn that invoked the tool and is waiting on the tool's result - so `agent_status` reads `idle` (or `blocked`, which the adapter also maps to `idle`), even though the pane's own rendered text keeps showing the harness's busy signature (the shared set in `bin/fm-tmux-lib.sh`; see [harness-busy-signatures.md](harness-busy-signatures.md)) the whole time, exactly as it would in a plain tmux pane.
