...
Staleness is judged on the pane body - the transcript above the trailing `FM_PANE_FOOTER_LINES` footer block - because a harness footer carries live counters that keep ticking on an abandoned pane.
`FM_PANE_FOOTER_LINES` governs only that body/footer split; the busy check reads its own, wider window (`FM_BUSY_TAIL_LINES`), because a harness can push its busy signature above the steady-state footer.
The signature set and that window are defined once in `bin/fm-tmux-lib.sh` and scanned through one shared helper, so the watcher, `bin/fm-crew-state.sh`, and the away-mode daemon cannot disagree about whether a pane is busy; each signature is an empirical per-harness fact, measured and re-measured by the procedure in [harness-busy-signatures.md](harness-busy-signatures.md).
...
That same no-mistakes run already performs the shellcheck and behavior-test checks the fork's GitHub Actions CI would re-run, so a firstmate-repo PR is mergeable once no-mistakes is green and does not block on that redundant fork CI, while project PRs still honor their own CI.
