...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits and finished-but-unlanded PRs that remain idle past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A finished ship task whose PR is still open reaches the same bounded cadence without declaring anything: its last status line is a terminal `done:` and `bin/fm-pr-check.sh` has already armed the merge poll that reports the landing, so its idle pane is expected rather than a possible wedge.
Both halves are derived from records their own paths already maintain, so nothing has to be set or cleared by hand, and the state ends on its own when the merged-PR retirement removes the poll - a task that wedges with its PR already landed alarms normally again.
Unfinished work is untouched: `blocked:`, `needs-decision:` and `failed:` still surface, as does a finished task whose merge poll has not been armed.
`bin/fm-classify-lib.sh` owns that predicate for both supervisors.
...
The daemon escalates captain-relevant events, plus a bounded recheck for a declared pause or a finished-but-unlanded PR that remains idle, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
