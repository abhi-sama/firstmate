...
config/pipeline-monitor  optional "on" opt-in to auto-opening the read-only validation monitor when a no-mistakes run starts; LOCAL, gitignored; absent or any other value keeps it manual-only; see docs/pipeline-monitor.md
...
Batch related small changes into one ship task and one PR by default, because every ship task pays for a full validation run; fan out only for genuinely independent work or a stated speed need, never to split related trivia.
...
When `config/pipeline-monitor` is `on`, also run `bin/fm-pipeline-monitor.sh <id>` right after triggering validation; it is a read-only observer, so a missing or failed monitor never blocks or changes the run (docs/pipeline-monitor.md).
Fit each repo's gate depth once in that repo's own committed no-mistakes config rather than re-judging it per change: a repo with no real test suite pins a fast deterministic test command so the gate does not spend a freeform validation agent proving nothing.
...
Run `bin/fm-pr-check.sh <id> <PR url>` - it records `pr=` and the forge's `pr_head=` when available in the task's meta and arms the watcher's merge poll, which is also what lets the finished task's idle endpoint stay quiet until the PR lands instead of repeatedly surfacing as a possible wedge.
