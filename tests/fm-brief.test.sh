...
test_ship_modes_generate_clean_briefs() {
  ...
  for id_mode in "brief-nomistakes-a1:no-mistakes" "brief-directpr-a2:direct-PR" "brief-localonly-a3:local-only"; do
    ...
    assert_grep "independently and adversarially reviewed" "$brief" \
      "$id: brief missing the reviewer-awareness statement"
    assert_no_grep "EOF" "$brief" "$id: brief leaked a heredoc EOF marker (unterminated heredoc)"
  done
  pass "fm-brief.sh: no-mistakes/direct-PR/local-only briefs generate cleanly"
}

# Scout briefs deliver a report, not shipped code, and their own definition of
# done already demands stand-alone, evidence-backed findings. The Quality bar is
# ship-specific scaffolding - it names no-mistakes' review gate and pre-merge
# review - so a scout brief must not carry it.
test_scout_brief_has_no_quality_bar_section() {
  local home id brief
  home="$TMP_ROOT/scout-quality-bar-home"
  write_registry "$home"
  id="brief-scout-c1"
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" some-proj --scout >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  assert_present "$brief" "scout brief was not scaffolded"
  assert_no_grep "# Quality bar" "$brief" \
    "scout brief unexpectedly got the ship-specific Quality bar section"
